...
def _get_reg_vals(binary_input_byte):
    ...
    try:
        if USE_ANGR:
            ...
            r = CustomRunner(binary, payload=test_input, ids_rules=glob_ids_rules)
        if not r.crash_mode:
            return [c, None]
        else:
            reg_vals = dict()
...
class Type2CrashFuzzer(object):
    def __init__(self, binary, crash=None, ids_rules=None, time_limit=None):
        ...
        if USE_ANGR:
            ...
            r = CustomRunner(self.binaries, payload=self.crash, record_stdout=True, ids_rules=glob_ids_rules)
        if not r.crash_mode:
    ...
    def _get_reg_eqn(self):
        ...
        if r.crashing_inst is None or len(r.crashing_inst) == 0:
            raise CrashFuzzerException("no instruction found")
        inst = r.crashing_inst
        if "[" not in inst or "]" not in inst.split("[")[1]:
            raise CrashFuzzerException("can't see the read")
        eqn = inst.split("[")[1].split("]")[0]
        return eqn
        ...
        for reg in regs_to_check:
            ...
            if len(new_stdout) > len(self.orig_stdout):
                ...
                if leak_idx is None:
                    ...
                    l.warning("need to send to colorguard")
    ...
    def get_leaking_payload(self):
        if self._raw_payload is None:
            raise CrashFuzzerException("no leaking payload")
